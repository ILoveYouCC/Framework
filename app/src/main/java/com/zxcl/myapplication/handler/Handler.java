...
public class Handler {
    ...
    /**
     * 静态内部类，相当于一个额外的类，只不过在外部类加载的时候他也同时被加载。
     *
     * 内部类：可以直接使用外部类的变量(实例和静态)
     *
     * 匿名内部类
     *
     * 实现多线程的方式之一：实现 Runnable 接口，重写 run 方法
     */
            ...
            synchronized (this) {
                if (timeout > 0) {
                    ...
                    while (!mDone) {
                        ...
                        try {
                            // wait() 等待方法 Object 类的实例方法
                            // 用来阻塞线程，但是会释放资源
                            // sleep() 睡眠方法 Thread 类的静态方法
                            // 也是用来阻塞线程，但是会一直持有资源
